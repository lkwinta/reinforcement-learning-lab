...
class MCTSNode:
    ...
    def select(self, final=False) -> tuple[int, int]:
        ...
        if final:
            counts = {move: child.times_chosen for move, child in self.children.items()}
    ...
    def backpropagate(self, winner: Colour) -> None:
        ...
        self.times_chosen += 1
        if self.current_player.flip() == winner:
            self.value += (1 - self.value) / self.times_chosen
        else:
            self.value += (0 - self.value) / self.times_chosen
...
def round(args):
    ...
    for _ in range(50):
        board = Board(7, 5)
        red_player = MCTSPlayer(*red_params)
        blue_player = MCTSPlayer(*blue_params)
        game = Game(red_player, blue_player, board)
        game.run(verbose=False)
    ...
    return {
        ...
        "winner": game.winner,
        "red_won": game.winner == Colour.RED,
