...
def main() -> None:
    ...
    min_result = min([(alpha, penalty, step_no) for step_no in results.keys() for alpha, penalty in results[step_no]], key=lambda x: x[1])

    for step_no in sorted(results.keys()):
        alphas, penalties = zip(*sorted(results[step_no]))

        plt.plot(alphas, penalties, label=f'step_no={step_no}', marker='o', markersize=3)
