...
def draw_episode(track: np.ndarray, positions: list[problem.Position], episode: int, plots_path: str) -> None:
    ax = plt.axes()
    ax.imshow(track)
    for i in range(len(positions) - 1):
        begin, end = positions[i], positions[i+1]
        draw_arrow(ax, begin, end)
    plt.savefig(f'{plots_path}/track_{episode}.png', dpi=300)
    plt.clf()


def draw_penalties_plot(penalties: list[int], window_size: int, episode: int, plots_path: str) -> None:
    means = [np.mean(penalties[i:i+window_size]) for i in range(len(penalties) - window_size)]
    ax = plt.axes()
    ax.plot(means)
    plt.savefig(f'{plots_path}/penalties_{episode}.png', dpi=300)
    plt.clf()
